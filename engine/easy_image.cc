...
void img::EasyImage::draw_zbuf_line(unsigned int x0, unsigned int y0, double z1, unsigned int x1, unsigned int y1, double z2, img::Color &color, ZBuffer &zbuffer) {
    assert(x0 < this->width && y0 < this->height);
    assert(x1 < this->width && y1 < this->height);
    ...
    if (x0 == x1) {
        ...
        a = max(y0, y1) - min(y0, y1);

//        for (unsigned int i = std::min(y0, y1); i <= std::max(y0, y1); i++) { // default functie
//            (*this)(x0, i) = color;
//        }
        for (double i = a; i >= 0; i--) { // zie cursus, beginnen bij i = a, dan i = a-1, ...
            double p = i / max(a,(unsigned int)1); // stel a = 0 => we pakken 1
            double One_Z = p/z1 + (1-p)/z2;

            unsigned int y = round(p*y0 + (1-p)*y1); // tip van leerkracht
            if (One_Z < zbuffer.pixelArray[x0][y]) {
                (*this)(round(x0), y) = color;
                zbuffer.pixelArray[x0][y] = One_Z;
            }
        }
    }
    else if (y0 == y1) {
...
//        }
        for (double i = a; i >= 0; i--) {
            ...
            unsigned int x = round(p*x0 + (1-p)*x1); // tip van leerkracht
            if (One_Z < zbuffer.pixelArray[x][y0]) {
                (*this)(x, round(y0)) = color;
                zbuffer.pixelArray[x][y0] = One_Z;
            }
        }
    }
    else {
        if (x0 > x1) { // niet zo belangrijk
            //flip points if x1>x0: we want x0 to have the lowest value
            std::swap(x0, x1);
            std::swap(y0, y1);
        }

        double m = ((double) y1 - (double) y0) / ((double) x1 - (double) x0); // rico
        if (-1.0 <= m && m <= 1.0) { // beetje schuin
            a = max(x0, x1) - min(x0, x1);
//            for (unsigned int i = 0; i <= (x1 - x0); i++) {
//                (*this)(x0 + i, (unsigned int) round(y0 + m * i)) = color;
//            }
            for (double i = a; i >= 0; i--) {
                ...
                unsigned int x = round(p*x0 + (1-p)*x1); // tip van leerkracht
                unsigned int y = round(p*y0 + (1-p)*y1);
                if (One_Z < zbuffer.pixelArray[x][y]) {
                    (*this)(x, y) = color;
                    zbuffer.pixelArray[x][y] = One_Z;
                }
            }
        }
        else if (m > 1.0) { // zeer schuin naar rechts boven
            a = max(y0, y1) - min(y0, y1);
...
//            }
        ...
        else if (m < -1.0) { // zeer schuin naar links beneden
            a = max(y0, y1) - min(y0, y1);
