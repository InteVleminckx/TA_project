...
namespace
{
	...
	// tekens <== stellen de veranderingen voor (namelijk de toegevoegde for loops), het idee om deze for loops
	// toe te voegen om het probleem op te lossen als je de opslagcontainer voor de rules verandert, heb ik gekregen van een medestudent.
	{
	...
	void parse_rules(std::set<char> const& alphabet, map<char, vector<std::pair<std::string,double>>>& rules, stream_parser& parser, bool parse2D) //
	{
		parser.skip_comments_and_whitespace();
		...
		parser.skip_comments_and_whitespace();
		rules.clear();
		double weight; // = percentage van voorkomen van rule
		char c = parser.getChar();
		while (true)
		{
			if (!std::isalpha(c))
...
//				throw LParser::ParserException(std::string("Double entry '") + c + "' in rules specification ", parser.getLine(), parser.getCol());
			...
			//rules.emplace_back(alphabet_char, make_pair(rule, 1));
			parser.skip_comments_and_whitespace();
            //c = parser.getChar();

			//

            if (parser.peekChar() == 48) { // als het character een 0 is
            ...
            rules[alphabet_char].push_back(make_pair(rule, weight));
			//
            parser.skip_comments_and_whitespace();
            c = parser.getChar();
...
vector<pair<std::string, double>> const& LParser::LSystem::get_replacement(char c) const
{
//	assert(get_alphabet().find(c) != get_alphabet().end());
